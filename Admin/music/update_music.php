<?php
    ...
    $id = $_GET['id'];
    $sql = "SELECT m.id, m.title, m.artist, m.category_id, m.description, m.banner_path, m.audio_path, m.status, c.name AS category_name
    FROM `music_list` AS m JOIN `category_list` AS c ON m.category_id = c.category_id WHERE m.id = $id";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $sql = "UPDATE music_list
                SET title = '$title', artist = '$artist', category_id = $category_id, description = '$description', banner_path = '$full_banner_path', audio_path = '$full_audio_path', status = $status
                WHERE id = $id";
...
<head>
    ...
    <script src="../../library/update_music.js"></script>
</head>
                ...
                <div class="container-fluid">
                    ...
                    <div class="card card-outline rounded-0 card-purple">
                        <div class="card-header">
                            <h3 class="card-title">Update Music Entry</h3>
                        ...
                        <div class="card-body">
                            <div class="container-fluid">
                                <form action="" id="update-form" method="post" enctype="multipart/form-data">
                                    <input type="hidden" name ="id" value="<?php echo $songs['id']; ?>">
                                    <div class="form-group">
                                        <label for="title" class="control-label">Title</label>
                                        <input type="text" name="title" id="title" class="form-control form-control-sm rounded-0" value="<?php echo htmlspecialchars($songs['title']); ?>"  required/>
                                    </div>
                                    <div class="form-group">
                                        <label for="artist" class="control-label">Artist</label>
                                        <input type="text" name="artist" id="artist" class="form-control form-control-sm rounded-0" value="<?php echo htmlspecialchars($songs['artist']); ?>"  required/>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select name="category_id" id="category_id" class="form-control form-control-sm rounded-0" required>
                                            ...
                                            <?php foreach ($result as $category): ?>
                                                <option value="<?= $category['category_id'] ?>" <?= ($category['category_id'] == $songs['category_id']) ? 'selected' : '' ?>>
                                                    <?= $category['name'] ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea rows="3" name="description" id="description" class="form-control form-control-sm rounded-0" required><?php echo $songs['description']; ?></textarea>
                                    </div>
                                    <div class="form-group">
                                        <label for="" class="control-label">Music Banner</label>
                                        <div class="custom-file">
                                        <input type="file" class="custom-file-input rounded-circle" id="customFile2" name="banner_path" value="" onchange="displayBanner(this,$(this))" required>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="custom-file mb-2">
                                            <input type="file" class="custom-file-input rounded-circle" id="customFile2" name="audio_path" accept="audio/*" onchange="displayAudioName(this,$(this))" required>
                                            <label class="custom-file-label" for="customFile2">Choose file</label>
                                        </div>
                        ...
                        <div class="card-footer py-2 text-center">
                            <button class="btn btn-primary rounded-0" type="submit" form="update-form">Save</button>
                        </div>
                    </div>
                </div>
            </section>
